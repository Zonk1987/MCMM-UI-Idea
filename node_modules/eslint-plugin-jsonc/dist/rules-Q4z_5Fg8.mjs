...
var JSONElementData = class {
	...
	*/
	getValueForKey(key) {
		const cache = this.cachedKeyValues ??= /* @__PURE__ */ new Map();
		if (cache.has(key)) return cache.get(key);
		let result;
		const node = this.node;
		if (node && node.type === "JSONObjectExpression") {
			for (const prop of node.properties) if (getPropertyName$1(prop) === key) {
...
*/
function buildValidatorFromType$1(order, insensitive, natural, key) {
	...
	*/
	function compare(aVal, bVal) {
		...
		if (type && type === getJSONPrimitiveType(bVal)) return compareValue([aVal, bVal]);
		return true;
	}
	return (a, b) => {
		if (key) {
	...
	return options.map((opt) => {
		...
		if (!Array.isArray(order)) {
			...
			return {
				...
				isValidOrder: buildValidatorFromType$1(type, insensitive, natural, key),
				orderText(data) {
		...
		for (const o of order) if (typeof o === "string") parsedOrder.push({
			...
			parsedOrder.push({
				test: (v) => {
					...
					return valuePattern ? Boolean(getJSONPrimitiveType(v.value)) && valuePattern.test(String(v.value)) : true;
				},
				isValidNestOrder: buildValidatorFromType$1(type, insensitive, natural, itemKey)
			});
		}
		return {
			isTargetArray,
		...
		function isTargetArray(data) {
			...
			while (p) {
				if (p.type === "JSONProperty") {
					const name = getPropertyName$1(p);
