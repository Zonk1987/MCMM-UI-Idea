...
export function playersApp() {
  return {
    ...
    playersData: {
      'minecraft01': {
        online: [
          { name: 'Steve_Gaming',  uuid: 'a8d3f1b2-e4c5-4a0d-9b6e-1234567890ab', ping: 24,  playtime: '14h 22m', gamemode: 'survival',  isOp: true },
          { name: 'CreeperSlayer', uuid: 'b9e4c2a3-f5d6-4b1e-ac7f-2345678901bc', ping: 67,  playtime: '3h 8m',   gamemode: 'survival',  isOp: false },
          { name: 'DiamondMiner99',uuid: 'c0f5d3b4-a6e7-4c2f-bd80-3456789012cd', ping: 142, playtime: '1h 55m',  gamemode: 'creative',  isOp: false },
        ],
        whitelist: [
        ...
        bans: [
          { name: 'Griefer_666',   uuid: 'f3c8a6e7-d9b0-4f5c-e013-6789012345f0', reason: 'Griefing und Belästigung', date: '2026-06-15', duration: 'Permanent' },
          { name: 'XrayHacker123', uuid: 'a4d9b7f8-e0c1-4a6d-f124-7890123456a1', reason: 'X-Ray Client', date: '2026-06-28', duration: '30 Tage' },
        ],
        chatlog: [
          ...
          { time: '13:15:02', player: 'DiamondMiner99', msg: 'Tp zu mir, ich bau gerade einen neuen Palast' },
      ...
      'minecraft02': {
        online: [
          { name: 'PixelArtist', uuid: 'e5f0c8a9-1b2c-3d4e-5f6a-7b8c9d0e1f2a', ping: 15, playtime: '2h 5m', gamemode: 'creative', isOp: true },
        ],
        whitelist: [
    ...
    get serverLabel() {
      ...
      return this.servers.find(s => s.containerId === this.state.server)?.serverName || this.state.server;
    },
    ...
    kickPlayer(player) {
      ...
      if(typeof showToast === 'function') showToast(`${player} ${typeof t === 'function' ? (t('was_kicked') || 'wurde gekickt') : 'wurde gekickt'}`, 'info');
    },
    ...
    confirmBan() {
      ...
      if(typeof showToast === 'function') showToast(`${player} ${typeof t === 'function' ? (t('was_banned') || 'wurde gebannt') : 'wurde gebannt'}`, 'error');
    },

    unbanPlayer(player) {
      if(!this.activeData) return;
      this.activeData.bans = this.activeData.bans.filter(b => b.name !== player);
      if(typeof showToast === 'function') showToast(`${player} ${typeof t === 'function' ? (t('was_unbanned') || 'wurde entbannt') : 'wurde entbannt'}`, 'success');
    },
    ...
    opPlayer(player) {
      ...
      if(p) {
        p.isOp = true;
        if(typeof showToast === 'function') showToast(`${player} ${typeof t === 'function' ? (t('now_has_op') || 'hat jetzt OP-Rechte') : 'hat jetzt OP-Rechte'}`, 'success');
      }
    },

    tpPlayer(player) {
      if(typeof showToast === 'function') showToast(`${typeof t === 'function' ? (t('tp_to') || 'Teleportiere zu') : 'Teleportiere zu'} ${player}...`, 'info');
    },

    msgPlayer(player) {
      const msg = prompt(`${typeof t === 'function' ? (t('msg_to') || 'Nachricht an') : 'Nachricht an'} ${player}:`);
      if (msg?.trim() && typeof showToast === 'function') {
        showToast(`${typeof t === 'function' ? (t('msg_sent') || 'Nachricht an') : 'Nachricht an'} ${player} ${typeof t === 'function' ? (t('sent') || 'gesendet') : 'gesendet'}`, 'success');
      }
    },
    ...
    toggleGamemode(player) {
      ...
      if(typeof showToast === 'function') showToast(`${player}: Gamemode → ${this.getGmLabel(p.gamemode)}`, 'info');
    },

    getGmLabel(gm) {
      const GM_LABEL = { survival: 'Survival', creative: 'Creative', adventure: 'Adventure', spectator: 'Spectator' };
    ...
    removeWhitelist(player) {
      ...
      if(typeof showToast === 'function') showToast(`${player} ${typeof t === 'function' ? (t('removed_from_whitelist') || 'von Whitelist entfernt') : 'von Whitelist entfernt'}`, 'info');
    },

    addWhitelist() {
       const name = prompt(`${typeof t === 'function' ? (t('enter_player_name') || 'Spielername eingeben:') : 'Spielername eingeben:'}`);
       if (name?.trim() && this.activeData) {
         this.activeData.whitelist.push({ name: name.trim(), uuid: 'neu-' + Date.now() });
         if(typeof showToast === 'function') showToast(`${name.trim()} ${typeof t === 'function' ? (t('added_to_whitelist') || 'zur Whitelist hinzugefügt') : 'zur Whitelist hinzugefügt'}`, 'success');
       }
    },
    ...
    sendChat() {
      ...
      if(typeof showToast === 'function') showToast(`${typeof t === 'function' ? (t('cmd_sent') || 'Befehl gesendet:') : 'Befehl gesendet:'} ${this.state.chatInput.trim()}`, 'success');
...
export function playerAvatarError(img) {
  img.onerror = null;
  img.src = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%234b5563"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>';
};
